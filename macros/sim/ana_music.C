...
void ana_music(const float nev = -1)
{
    ...
    auto* branchMCTrack = dynamic_cast<TBranch*>(tree->GetBranch("MCTrack"));
    branchMCTrack->SetAddress(&MCTrackCA);

    // MUSIC Point Data
    auto musicpoint = new TClonesArray("R3BMusicPoint");
    auto* branchMusicData = dynamic_cast<TBranch*>(tree->GetBranch("MusicPoint"));
    if (branchMusicData)
        branchMusicData->SetAddress(&musicpoint);

    // Histograms
    auto h2_theta_ek = new TH2F("h2_Theta_Ek", "Theta vs Ek", 1000, 0., 150., 1200, 0., 1200.);
